...
def multimodal_pair_collate_fn(batch):
    ...
    # Quando pair=True com grupos, cada item[0] e item[1] é uma lista de amostras
    # (um grupo inteiro). Precisamos achatar antes de passar para o collate.
    if low_batch and isinstance(low_batch[0], list):
        low_batch  = [sample for group in low_batch  for sample in group]
        high_batch = [sample for group in high_batch for sample in group]
