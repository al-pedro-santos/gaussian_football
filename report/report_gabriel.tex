...
Gostaríamos de que fosse possível a extração dos melhores momentos de uma partida de futebol a partir do vídeo e áudio brutos da transmissão limpa de uma partida em específico. Isso facilitaria o trabalho de editores de vídeo, comentadores e espectadores que não puderam ver o jogo de acompanharem os momentos marcantes de um jogo de futebol.
...
Cada partida possui anotações de momentos-chave fornecidas pelo dataset SoccerNet: gols, cartões, defesas, escanteios, entre outros. Considere G o conjunto dos gols em uma partida específica. Para cada gol $g \in G$ anotado no instante $t_{\text{g}}$, associa-se uma curva gaussiana centrada nesse instante de tempo. O \textit{arousal score} de qualquer instante $t$ que não seja um instante de gol na partida é definido por:

\begin{equation}
    a(t) = \argmax_{g \in G} \Bigg[ \exp\left(-\frac{(t - t_{\text{g}})^2}{2\sigma^2}\right)\Bigg]
    \label{eq:arousal}
\end{equation}

\noindent onde:
\begin{itemize}
    \item $t_{\text{g}}$ é o instante do gol $g$, onde o score atinge seu pico máximo $a(t_{\text{g}}) = 1{,}0$;
    \item $\sigma$ controla o raio de influência temporal do gol, determinado pelos parâmetros \texttt{--window\_before} e \texttt{--window\_after};
    \item o score decai simetricamente à medida que $t$ se afasta de $t_{\text{g}}$, tendendo a zero fora da janela de highlight.
\end{itemize}
