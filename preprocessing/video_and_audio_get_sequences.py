...
class VideoAudioGetSequences:
    ...
    def has_overlap(self, start, end, intervals):
        ...
        for h_start, h_end in intervals:
            if self.time_to_seconds(start) < self.time_to_seconds(
                h_end
    ...
    def save_video_clip(
        self, video, start, end, output_path, fps, grayscale=False, with_audio=False
    ):
    ...
    def preprocess(
        ...
        with VideoFileClip(vid_path) as video:
            ...
            for idx, (start, end) in enumerate(highlights):
                output_path_video = (
                    highlight_video_dir / f"{video_name}_highlight_{idx}.{video_format}"
                )
                ...
                if has_audio:
                    output_path_mel = (
                        highlight_mel_dir / f"{video_name}_highlight_{idx}.npy"
                    )
                    self.save_mel_spectrogram(audio, start, end, output_path_mel)

                    if save_audio:
                        output_path_audio = (
                            highlight_audio_dir / f"{video_name}_highlight_{idx}.wav"
                        )
            ...
            while count < qtd_negativos and attempts < 1000:
                ...
                output_path_video = (
                    no_highlight_video_dir
                    / f"{video_name}_no_highlight_{count}.{video_format}"
                )
                self.save_video_clip(
                    video,
                    start,
                ...
                output_path_mel = (
                    no_highlight_mel_dir / f"{video_name}_no_highlight_{count}.npy"
                )
                self.save_mel_spectrogram(audio, start, end, output_path_mel)
                ...
                if has_audio:
                    ...
                    if save_audio:
                        output_path_audio = (
                            no_highlight_audio_dir
                            / f"{video_name}_no_highlight_{count}.wav"
                        )
                        self.save_audio_clip(audio, start, end, output_path_audio)
                count += 1
    ...
    def save_segments(
        ...
        if split in splits_path:
            ...
            raise ValueError(
                f"Split '{split}' inválido. Escolha entre 'train', 'val' ou 'test'."
            )
        ...
        with VideoFileClip(vid_path) as video:
            ...
            for idx, (start, end) in enumerate(intervals):
                ...
                if self.is_silent(
                    audio_array
                ):  # não vai processar e salvar mel spectograma e audio
                    continue
