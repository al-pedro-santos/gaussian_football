...
def balanced_df_window(
    ...
    balanced_df : pd.DataFrame
        ...
        preservadas, mais as colunas 'window_category' e 'window_id'.
        'window_id' e um identificador unico GLOBALMENTE (combina game_col +
        half + event_id_col), diferente de event_id_col cru -- este ultimo
        se repete entre jogos distintos (ex: 'goal_0' existe em centenas de
        jogos) e por isso NAO deve ser usado como column_groups_id no
        loader. Use 'window_id' para agrupar/parear janelas no loader
        (MultiModalDataset com groups=True, column_groups_id='window_id').
        A janela de Background pareada com uma janela de referencia
        compartilha o mesmo 'window_id' dela. Ordenado por
    ...
    selected_background_window_ids = []  # window_id paralelo, mesma ordem/tamanho
    ...
    for ref in reference_windows:
        ...
        # window_id: identificador unico globalmente por janela de referencia
        # (necessario porque event_id_col cru, ex: 'goal_0', se repete entre
        # jogos diferentes -- sem o game_id/half embutidos, o loader
        # agruparia erroneamente janelas de jogos distintos como se fossem
        # a mesma). Atribuido com ou sem match, para que toda janela de
        # referencia no balanced_df tenha um window_id valido.
        window_id = f"{game_id}__{half}__{ref['event_id']}"
        ref["window_id"] = window_id

        if found:
            matched_count += 1
            # a janela de Background pareada compartilha o window_id da
            # janela de referencia correspondente
            selected_background_window_ids.append(window_id)
        else:
            unmatched_count += 1
    ...
    window_id_map = {}  # indice -> window_id (unico globalmente, game+half+event_id)

    for w in reference_windows:
        for i in w["indices"]:
            window_category_map[i] = w["category"]
            window_id_map[i] = w["window_id"]
        all_used_indices.append(w["indices"])

    for idx_arr, window_id in zip(bg_indices_all, selected_background_window_ids):
        for i in idx_arr:
            window_category_map[i] = category_of(background_label)
            window_id_map[i] = window_id
    ...
    balanced_df["window_id"] = balanced_df.index.map(window_id_map)
